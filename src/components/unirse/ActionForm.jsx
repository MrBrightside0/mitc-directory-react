  ...
  <div className="relative w-full group">
    ...
    <div className="relative">
      ...
      <input 
        ...
        onChange={onChange}
        required={required}
        className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm text-slate-900 placeholder-slate-400 text-base focus:bg-white" 
...
const ActionForm = () => {
  ...
  return (
    <section ref={sectionRef} className="bg-slate-50 py-12 md:py-24 md:-mt-10 relative z-20" id="formulario">
        <div className="reveal-form max-w-7xl mx-auto bg-white shadow-none md:shadow-2xl overflow-hidden border-y md:border border-slate-200 flex flex-col lg:flex-row relative">
            ...
            <div className="lg:w-5/12 bg-slate-900 text-white p-8 lg:p-14 flex flex-col justify-between relative overflow-hidden">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-600 blur-[120px] opacity-20 pointer-events-none translate-x-1/2 -translate-y-1/2"></div>
                <div className="relative z-10">
                    <div className="inline-block px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/10 text-indigo-200 text-xs font-bold uppercase tracking-widest mb-6">Membresía Business Connect</div>
                    ...
                    <ul className="space-y-8">
                        <li className="flex gap-5">
                            <div className="w-12 h-12 bg-indigo-500/20 flex items-center justify-center shrink-0 border border-indigo-500/30 text-indigo-300"><Users className="h-6 w-6" /></div>
                        ...
                        <li className="flex gap-5">
                            <div className="w-12 h-12 bg-emerald-500/20 flex items-center justify-center shrink-0 border border-emerald-500/30 text-emerald-300"><Building2 className="h-6 w-6" /></div>
            ...
            <div className="lg:w-7/12 p-6 md:p-14 bg-white">
                <div className="flex p-1.5 bg-slate-100/80 mb-10 max-w-md mx-auto md:mx-0">
                    <button onClick={() => setFormType('empresa')} className={`flex-1 py-3 px-4 text-sm font-bold transition-all flex items-center justify-center gap-2 ${formType === 'empresa' ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700'}`}><Mail className="h-4 w-4" /> Solicitud</button>
                    <button onClick={() => setFormType('agendar')} className={`flex-1 py-3 px-4 text-sm font-bold transition-all flex items-center justify-center gap-2 ${formType === 'agendar' ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700'}`}><Calendar className="h-4 w-4" /> Agendar Cita</button>
                </div>
                ...
                {formType === 'empresa' && (
                    ...
                    >
                        ...
                        <div className="pt-8 mt-4 border-t border-slate-100">
                            <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-xl shadow-indigo-200/50 hover:shadow-2xl hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-3 text-lg disabled:opacity-70 disabled:cursor-not-allowed">
                                {isSubmitting ? (
                ...
                {formType === 'agendar' && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16 px-4 flex flex-col items-center justify-center h-full">
                        <div className="w-28 h-28 bg-indigo-50 flex items-center justify-center mb-8 ring-8 ring-indigo-50/50 shadow-inner"><Calendar className="h-14 w-14 text-indigo-600" /></div>
