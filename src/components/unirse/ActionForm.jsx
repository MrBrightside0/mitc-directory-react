...
import { 
  Users, Building2, Mail, Calendar, User, Briefcase, Phone, MapPin, ArrowRight, Send 
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto bg-white md:rounded-[2.5rem] shadow-none md:shadow-2xl overflow-hidden border-y md:border border-slate-200 flex flex-col lg:flex-row relative">
            ...
            <div className="lg:w-5/12 bg-slate-900 text-white p-8 lg:p-14 flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="inline-block px-4 py-1.5 bg-white/10 backdrop-blur-md border border-white/10 rounded-lg text-indigo-200 text-xs font-bold uppercase tracking-widest mb-6">
                    ...
                    <ul className="space-y-8">
                        <li className="flex gap-5">
                            <div className="w-12 h-12 rounded-2xl bg-indigo-500/20 flex items-center justify-center shrink-0 border border-indigo-500/30 text-indigo-300">
                                <Users className="h-6 w-6" />
                            </div>
                            <div>
                                <h4 className="font-bold text-white text-lg">Networking C-Level</h4>
                                <p className="text-sm text-slate-400 leading-relaxed mt-1">Conecta con dueños y directores. Sin intermediarios.</p>
                            </div>
                        </li>
                        <li className="flex gap-5">
                            <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 flex items-center justify-center shrink-0 border border-emerald-500/30 text-emerald-300">
                                <Building2 className="h-6 w-6" />
                            </div>
                            <div>
                                <h4 className="font-bold text-white text-lg">Tu Oficina en el PIIT</h4>
                                <p className="text-sm text-slate-400 leading-relaxed mt-1">Impresiona a tus clientes. Salas de juntas y auditorios.</p>
                            </div>
                        </li>
                    </ul>
                </div>
                ...
                <div className="relative z-10 mt-16 pt-8 border-t border-white/10 hidden md:block">
                    ...
                    <div className="flex flex-wrap items-center gap-8 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
                        {PARTNER_LOGOS.map((logo, i) => (
                            <img key={i} src={logo} alt="Partner" className="h-8 w-auto object-contain brightness-0 invert hover:brightness-100 hover:invert-0 transition-all" />
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div className="lg:w-7/12 p-6 md:p-14 bg-white">
                ...
                <div className="flex p-1.5 bg-slate-100/80 rounded-2xl mb-10 max-w-md mx-auto md:mx-0">
                    <button 
                        ...
                        className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${formType === 'empresa' ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <Mail className="h-4 w-4" /> Solicitud
                    </button>
                    <button 
                        onClick={() => setFormType('agendar')}
                        className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${formType === 'agendar' ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                ...
                {formType === 'empresa' && (
                    ...
                    >
                        ...
                        <div className="pt-8 mt-4 border-t border-slate-100">
                            <button className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-xl shadow-indigo-200/50 hover:shadow-2xl hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-3 text-lg">
                                Iniciar Proceso <Send className="h-5 w-5" />
                            </button>
                            <p className="text-center text-xs text-slate-400 mt-4 px-4 leading-relaxed">
                                Tus datos están seguros. Al enviar aceptas ser contactado por el equipo comercial de MITC para validar tu perfil.
                            </p>
                        </div>
                    </motion.form>
                )}
                ...
                {formType === 'agendar' && (
                    ...
                    >
                        <div className="w-28 h-28 bg-indigo-50 rounded-full flex items-center justify-center mb-8 ring-8 ring-indigo-50/50 shadow-inner">
                        ...
                        <a 
                            ...
                            className="inline-flex items-center gap-3 px-10 py-5 bg-white border-2 border-indigo-600 text-indigo-700 font-bold rounded-xl hover:bg-indigo-600 hover:text-white transition-all text-lg shadow-lg hover:shadow-xl"
                        >
